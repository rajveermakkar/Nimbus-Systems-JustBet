...
export function ConfirmModal({ open, title = "Are you sure?", message = "", onConfirm, onCancel, confirmText = "Yes", cancelText = "Cancel" }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-purple-500/30 backdrop-blur-md rounded-2xl shadow-2xl p-8 max-w-sm w-full flex flex-col items-center border border-purple-200/30">
        ...
        <p className="text-white/90 text-center mb-6">{message}</p>
        <div className="flex gap-4 w-full justify-center">
          <button
            onClick={onConfirm}
