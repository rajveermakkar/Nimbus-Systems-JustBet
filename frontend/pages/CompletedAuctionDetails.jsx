...
function CompletedAuctionDetails() {
  ...
  const fetchAuctionDetails = async () => {
    try {
      ...
      console.log('Fetching auction details for:', { id, type });
      ...
      if (!auctionResponse.ok) {
        ...
        console.error('Auction response error:', errorText);
      ...
      // Fetch winner details if auction was sold
      if (auctionData.auction.status === 'closed' && auctionData.auction.current_highest_bidder_id) {
        try {
          const winnerUrl = `${import.meta.env.VITE_BACKEND_URL}/api/auth/user/${auctionData.auction.current_highest_bidder_id}`;
          console.log('Fetching winner from URL:', winnerUrl);
          ...
          if (winnerResponse.ok) {
            ...
            console.error('Winner response error:', winnerResponse.status);
          }
        } catch (err) {
      ...
      setToast({ show: true, message: err.message || 'Failed to load auction details', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
